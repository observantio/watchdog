...
def test_resolver_proxy_helper_methods(monkeypatch):
    service = ResolverProxyService()
    monkeypatch.setattr(config, "RESOLVER_PROXY_CACHE_TTL_SECONDS", 15)
    ...
    with pytest.raises(HTTPException, match="Missing Resolver signing key"):
...
@pytest.mark.asyncio
async def test_resolver_proxy_timeout_maps_to_504(monkeypatch):
    service = ResolverProxyService()
    monkeypatch.setattr(service, "write_audit", lambda **_kwargs: None)
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ALGORITHM", "HS256")
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ISSUER", "watchdog-main")
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_AUDIENCE", "resolver")
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_TTL_SECONDS", 120)
    monkeypatch.setattr(
        config,
        "get_secret",
        lambda key: {
...
@pytest.mark.asyncio
async def test_resolver_proxy_missing_service_token_and_generic_error(monkeypatch):
    service = ResolverProxyService()
    audits = []
...
@pytest.mark.asyncio
async def test_resolver_proxy_upstream_error_passthrough(monkeypatch):
    service = ResolverProxyService()
    monkeypatch.setattr(service, "write_audit", lambda **_kwargs: None)
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ALGORITHM", "HS256")
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ISSUER", "watchdog-main")
...
@pytest.mark.asyncio
async def test_resolver_proxy_does_not_cache_job_reads(monkeypatch):
    service = ResolverProxyService()
    monkeypatch.setattr(service, "write_audit", lambda **_kwargs: None)
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ALGORITHM", "HS256")
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ISSUER", "watchdog-main")
...
@pytest.mark.asyncio
async def test_resolver_proxy_cache_hit_and_invalid_json(monkeypatch):
    service = ResolverProxyService()
    monkeypatch.setattr(service, "write_audit", lambda **_kwargs: None)
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ALGORITHM", "HS256")
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ISSUER", "watchdog-main")
...
@pytest.mark.asyncio
async def test_resolver_proxy_collapses_inflight_reads(monkeypatch):
    service = ResolverProxyService()
    monkeypatch.setattr(service, "write_audit", lambda **_kwargs: None)
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ALGORITHM", "HS256")
    monkeypatch.setattr(config, "RESOLVER_CONTEXT_ISSUER", "watchdog-main")

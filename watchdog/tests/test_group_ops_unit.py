...
def test_group_helper_functions_and_access_rules(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(group_ops, "_prune_removed_member_notifier_group_shares", lambda **kwargs: (_ for _ in ()).throw(HTTPException(status_code=502, detail="boom")))
...
def test_notifier_prune_http_branches(monkeypatch):
    monkeypatch.setattr(group_ops.config, "NOTIFIER_URL", "")
    monkeypatch.setattr(group_ops.config, "get_secret", lambda key: None)
    group_ops._prune_removed_member_notifier_group_shares(tenant_id="t1", group_id="g1", removed_user_ids=["u1"])

    monkeypatch.setattr(group_ops.config, "NOTIFIER_URL", "https://notify")
    monkeypatch.setattr(group_ops.config, "NOTIFIER_TIMEOUT_SECONDS", 2)

...
def test_resolver_alertmanager_and_agents_workflow(client, monkeypatch: pytest.MonkeyPatch) -> None:
    state = WorkflowState()
    patch_auth_service(monkeypatch, state)
    ...
    template_response = client.get("/api/resolver/analyze/config-template", headers=admin_headers)
    ...
    assert client.delete("/api/resolver/reports/report-1", headers=admin_headers).status_code == 200

    for path in [
        "/api/resolver/anomalies/metrics",
        "/api/resolver/anomalies/logs/patterns",
        "/api/resolver/anomalies/logs/bursts",
        "/api/resolver/anomalies/traces",
        "/api/resolver/correlate",
        "/api/resolver/topology/blast-radius",
        "/api/resolver/slo/burn",
        "/api/resolver/forecast/trajectory",
        "/api/resolver/causal/granger",
        "/api/resolver/causal/bayesian",
    ]:

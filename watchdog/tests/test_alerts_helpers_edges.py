...
async def test_find_silence_for_mutation_paths(monkeypatch):
    ...
    monkeypatch.setattr(helpers_mod.notifier_proxy_service._client, "request", raise_timeout)
    ...
    monkeypatch.setattr(helpers_mod.notifier_proxy_service._client, "request", raise_http)
    ...
    monkeypatch.setattr(helpers_mod.notifier_proxy_service._client, "request", error_response)
    ...
    monkeypatch.setattr(helpers_mod.notifier_proxy_service._client, "request", invalid_json_response)
...
async def test_webhook_route_enforces_security_and_forwards(monkeypatch):
    ...
    monkeypatch.setattr(helpers_mod.notifier_proxy_service, "forward", fake_forward)
    handler = helpers_mod.webhook_route("firing", "alerts.webhook", "alerts")

    response = await handler(_request(headers=[(b"x-watchdog-webhook-token", b"secret")]))

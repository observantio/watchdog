...
def _propagate_removed_member_group_shares(
    *,
    ...
    try:
        _prune_removed_member_notifier_group_shares(
            tenant_id=tenant_id,
            group_id=group_id,
...
def _prune_removed_member_notifier_group_shares(
    *,
    tenant_id: str,
    group_id: str,
    removed_user_ids: List[str],
    ...
    if not base_url or not service_token:
        logger.warning("Skipping Notifier group-share prune; Notifier URL or service token is missing")
        return
    ...
    try:
        with httpx.Client(timeout=float(config.NOTIFIER_TIMEOUT_SECONDS)) as client:
        ...
        if response.status_code >= 400:
            raise HTTPException(
                ...
                detail="Failed to propagate group-share revocation to Notifier",
            )

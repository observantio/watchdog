"""
Proxy client for forwarding alertmanager API calls to Notifier.
...
class NotifierProxyService(BaseProxyService):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            timeout=float(config.NOTIFIER_TIMEOUT_SECONDS),
            tls_enabled=bool(config.NOTIFIER_TLS_ENABLED),
    ...
    def _sign_context_token(
        ...
        *,
        ...
        claims = self._build_base_jwt_claims(
            ...
            issuer=config.NOTIFIER_CONTEXT_ISSUER,
            audience=config.NOTIFIER_CONTEXT_AUDIENCE,
        ...
        return self._encode_jwt(
            claims,
            key,
            str(config.NOTIFIER_CONTEXT_ALGORITHM or "HS256").strip(),
        )
    ...
    async def forward(
        ...
        *,
        ...
        if webhook_token:
            headers["x-watchdog-webhook-token"] = webhook_token

...
async def get_analyze_config_template(
    request: Request,
    current_user: TokenData = Depends(require_permission_with_scope(Permission.CREATE_RCA, "resolver")),
) -> AnalyzeConfigTemplateResponse:
...
async def get_analyze_job_result(
    ...
    try:
        ...
        if exc.status_code != status.HTTP_409_CONFLICT:
            raise
        # Resolver can briefly return 409 for a just-completed job before the

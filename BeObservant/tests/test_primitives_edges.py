...
def test_json_helpers_and_audit_utility_edges(monkeypatch):
    ...
    cookie_request = _request(headers=[(b"cookie", b"beobservant_token=cookie-token")])
...
def test_main_startup_initializes_database_and_auth(monkeypatch):
    ...
    monkeypatch.setattr(config_module.config, "DATABASE_URL", "postgresql://safeuser:safePass_123@db:5432/beobservant")
    ...
    assert calls == [
        ("init_database", "postgresql://safeuser:safePass_123@db:5432/beobservant", True),
        "init_db",
        "lazy_init",

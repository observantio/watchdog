...
from models.observability.becertain_models import AnalyzeJobStatus, AnalyzeJobSummary, AnalyzeProxyPayload, AnalyzeRequestPayload
from models.observability.loki_models import LogDirection, LogFilterRequest, LogSearchRequest
from routers.observability import becertain_router, loki_router
...
@pytest.mark.asyncio
async def test_becertain_router_remaining_wrappers_and_helpers(monkeypatch):
    current_user = _user()
    calls = []
    ...
    monkeypatch.setattr(becertain_router, "resolve_tenant_id", fake_resolve_tenant_id)
    monkeypatch.setattr(becertain_router, "correlation_id", lambda _request: "corr-1")
    monkeypatch.setattr(becertain_router, "inject_tenant", lambda payload, tenant_id: {**payload, "tenant_id": tenant_id})
    ...
    assert becertain_router._json_dict({"ok": True}) == {"ok": True}
    assert becertain_router._json_dict("x") == {}
    ...
    result = becertain_router._job_result_response_from_summary(summary)
    assert result.result is None
    assert result.status == AnalyzeJobStatus.COMPLETED

    created = await becertain_router.create_analyze_job(
        _request("/api/becertain/analyze/jobs"),
        AnalyzeRequestPayload(start=1, end=2, services=["api"]),
        current_user,
    )
    assert created.job_id == "job-1"

    listed = await becertain_router.list_analyze_jobs(
        _request("/api/becertain/analyze/jobs"),
    ...
    fetched = await becertain_router.get_analyze_job("job-1", _request(), current_user)
    assert fetched.status == AnalyzeJobStatus.COMPLETED

    payload = AnalyzeProxyPayload.model_validate({"service": "api"})
    wrappers = [
        (becertain_router.anomalies_log_patterns, "/api/v1/anomalies/logs/patterns"),
        (becertain_router.anomalies_log_bursts, "/api/v1/anomalies/logs/bursts"),
        (becertain_router.anomalies_traces, "/api/v1/anomalies/traces"),
        (becertain_router.correlate_signals, "/api/v1/correlate"),
        (becertain_router.topology_blast_radius, "/api/v1/topology/blast-radius"),
        (becertain_router.slo_burn, "/api/v1/slo/burn"),
        (becertain_router.forecast_trajectory, "/api/v1/forecast/trajectory"),
        (becertain_router.causal_granger, "/api/v1/causal/granger"),
        (becertain_router.causal_bayesian, "/api/v1/causal/bayesian"),
    ]
    for handler, path in wrappers:
        proxied = await handler(_request(path), payload, current_user)
        assert proxied["path"] == path

    assert (await becertain_router.ml_weights(_request("/api/becertain/ml/weights"), current_user))["path"] == "/api/v1/ml/weights"
    assert (await becertain_router.events_deployments(_request("/api/becertain/events/deployments"), current_user))["path"] == "/api/v1/events/deployments"

    async def fake_request_json_non_dict(**_kwargs):
        return SimpleNamespace()

    monkeypatch.setattr(becertain_router.becertain_proxy_service, "request_json", fake_request_json_non_dict)
    assert await becertain_router._proxy_post(
        request=_request(),
        current_user=current_user,

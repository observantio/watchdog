...
from db_models import AlertRule as AlertRuleDB, AlertIncident as AlertIncidentDB, NotificationChannel as NotificationChannelDB, AuditLog, Group, User
...
def _resolve_rule_by_alertname(
    ...
    if not alertname:
        return None
    org_id_hint = str(labels.get("org_id") or labels.get("orgId") or labels.get("tenant") or labels.get("product") or "").strip()
    try:
        q = db.query(AlertRuleDB).filter(AlertRuleDB.tenant_id == tenant_id, AlertRuleDB.name == alertname)
        if org_id_hint:
            return q.filter((AlertRuleDB.org_id == org_id_hint) | (AlertRuleDB.org_id.is_(None))).order_by(AlertRuleDB.org_id.desc()).first()
        return q.first()
...
class DatabaseStorageService:
    ...
    def _rule_to_pydantic(self, r: AlertRuleDB) -> AlertRulePydantic:
        return AlertRulePydantic(
            ...
            visibility=r.visibility or "private", shared_group_ids=_get_shared_group_ids(r),
        )
    ...
    def sync_incidents_from_alerts(self, tenant_id: str, alerts: List[Dict[str, Any]], resolve_missing: bool = True) -> None:
        ...
        with get_db_session() as db:
            for alert in alerts or []:
                ...
                if not fingerprint:
                    stable_blob = json.dumps(
                        {"alertname": labels.get("alertname") or "", "severity": labels.get("severity") or "", "labels": labels, "annotations": annotations},
                ...
                incident = db.query(AlertIncidentDB).filter(
                    AlertIncidentDB.tenant_id == tenant_id, AlertIncidentDB.fingerprint == fingerprint
                ).first()
            ...
            if resolve_missing:
                for incident in db.query(AlertIncidentDB).filter(AlertIncidentDB.tenant_id == tenant_id, AlertIncidentDB.status == "open").all():
    ...
    def list_incidents(
        ...
        group_id: Optional[str] = None, limit: Optional[int] = None, offset: int = 0,
    ...
    def get_incident_for_user(
        self, incident_id: str, tenant_id: str, user_id: Optional[str] = None,
        group_ids: Optional[List[str]] = None, require_write: bool = False,
    ) -> Optional[AlertIncidentPydantic]:
        group_ids = group_ids or []
        with get_db_session() as db:
            incident = db.query(AlertIncidentDB).filter(
                AlertIncidentDB.id == incident_id, AlertIncidentDB.tenant_id == tenant_id
            ).first()
            if not incident:
                return None
            ...
            if user_id:
                ...
                if not _has_access(visibility, creator_id, user_id, _safe_group_ids(meta), group_ids, require_write=require_write):
                    return None

            return self._incident_to_pydantic(incident)

    def _is_assignee_allowed(
        self, db: Session, *, tenant_id: str, actor_user_id: str, assignee_id: Optional[str],
        visibility: str, shared_group_ids: List[str],
    ) -> bool:
        if not assignee_id:
            return True
        ...
        assignee_user = db.query(User).options(joinedload(User.groups)).filter(
            User.id == assignee_id, User.tenant_id == tenant_id, User.is_active.is_(True)
        ).first()
    ...
    def filter_alerts_for_user(
        self, tenant_id: str, user_id: str, group_ids: Optional[List[str]], alerts: List[Dict[str, Any]],
    ) -> List[Dict[str, Any]]:
        ...
        with get_db_session() as db:
            ...
            for alert in alerts:
                ...
                org_id_hint = str(labels.get("org_id") or labels.get("orgId") or labels.get("tenant") or labels.get("product") or "").strip()
                candidates = (
                    db.query(AlertRuleDB).options(joinedload(AlertRuleDB.shared_groups))
                ...
                if any(
                    _has_access(_normalize_visibility(getattr(r, "visibility", None)), getattr(r, "created_by", None), user_id, _get_shared_group_ids(r), user_group_ids)
                    for r in candidates
                ):
    ...
    def update_incident(
        self, incident_id: str, tenant_id: str, user_id: str, payload: AlertIncidentUpdateRequest,
    ) -> Optional[AlertIncidentPydantic]:
        with get_db_session() as db:
            incident = db.query(AlertIncidentDB).filter(
                AlertIncidentDB.id == incident_id, AlertIncidentDB.tenant_id == tenant_id
            ...
            if payload.assignee is not None:
                ...
                if not self._is_assignee_allowed(db, tenant_id=tenant_id, actor_user_id=user_id, assignee_id=requested_assignee, visibility=visibility, shared_group_ids=shared_group_ids):
                    if visibility == "private":
            ...
            for meta_key, payload_attr in [("jira_ticket_key", "jira_ticket_key"), ("jira_ticket_url", "jira_ticket_url"), ("jira_integration_id", "jira_integration_id")]:
    ...
    def get_public_alert_rules(self, tenant_id: str) -> List[AlertRulePydantic]:
        with get_db_session() as db:
            rules = db.query(AlertRuleDB).options(joinedload(AlertRuleDB.shared_groups)).filter(
                AlertRuleDB.tenant_id == tenant_id, AlertRuleDB.visibility == "public", AlertRuleDB.enabled.is_(True)
            ).all()
    ...
    def get_alert_rules_for_org(self, tenant_id: str, org_id: str) -> List[AlertRulePydantic]:
        with get_db_session() as db:
            rules = db.query(AlertRuleDB).options(joinedload(AlertRuleDB.shared_groups)).filter(
                AlertRuleDB.tenant_id == tenant_id, AlertRuleDB.org_id == org_id
            ).all()
            return [self._rule_to_pydantic(r) for r in rules]
    ...
    def get_alert_rules_with_owner(
        ...
        with get_db_session() as db:
            ...
            return [
                (self._rule_to_pydantic(r), r.created_by) for r in rules
                if _has_access(r.visibility or "private", r.created_by, user_id, _get_shared_group_ids(r), group_ids)
            ]

    def get_alert_rule_raw(self, rule_id: str, tenant_id: str):
        with get_db_session() as db:
            return db.query(AlertRuleDB).options(joinedload(AlertRuleDB.shared_groups)).filter(
                AlertRuleDB.id == rule_id, AlertRuleDB.tenant_id == tenant_id
            ).first()

    def get_alert_rule(
        self, rule_id: str, tenant_id: str, user_id: str, group_ids: Optional[List[str]] = None,
    ) -> Optional[AlertRulePydantic]:
        group_ids = group_ids or []
        with get_db_session() as db:
            r = db.query(AlertRuleDB).options(joinedload(AlertRuleDB.shared_groups)).filter(
                AlertRuleDB.id == rule_id, AlertRuleDB.tenant_id == tenant_id
            ).first()
    ...
    def create_alert_rule(
        self, rule_create: AlertRuleCreate, tenant_id: str, user_id: str, group_ids: Optional[List[str]] = None,
    ) -> AlertRulePydantic:
        with get_db_session() as db:
            rule = AlertRuleDB(
    ...
    def get_notification_channels(
        ...
        with get_db_session() as db:
            ...
            return [
                ...
                if _has_access(ch.visibility or "private", ch.created_by, user_id, _get_shared_group_ids(ch), group_ids)
            ]

    def get_notification_channel(
        self, channel_id: str, tenant_id: str, user_id: str, group_ids: Optional[List[str]] = None,

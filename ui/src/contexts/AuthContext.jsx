...
const getOidcRedirectUri = () => `${globalThis.location.origin}${OIDC_REDIRECT_PATH}`
...
const isOnOidcCallback = () => globalThis.location.pathname === OIDC_REDIRECT_PATH
...
export function AuthProvider({ children }) {
  ...
  const startOIDCLogin = useCallback(async () => {
    ...
    if (!resp?.authorization_url) throw new Error('OIDC authorization URL was not returned by the server')
    if (!resp?.transaction_id) throw new Error('OIDC transaction was not returned by the server')
  ...
  const finishOIDCLogin = useCallback(
    async ({ code, state }) => {
      ...
      if (!state || !expectedState || state !== expectedState) throw new Error('Invalid OIDC state')

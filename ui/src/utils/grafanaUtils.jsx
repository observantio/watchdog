...
export function overrideDashboardDatasource(dashboard, datasourceUid, availableDatasources = [], injectTemplating = true) {
  if (!dashboard || !datasourceUid) return dashboard

  const dsObj = availableDatasources.find((d) => String(d.uid) === String(datasourceUid))
  ...
  if (injectTemplating) {
    if (!out.templating || typeof out.templating !== 'object') {
      out.templating = { list: [ { name: 'ds_default', label: 'Datasource', type: 'datasource', current: { text: dsName, value: datasourceUid } } ] }
    }
  }
    ...
    for (const key of Object.keys(val)) {
      ...
      if (key === 'targets' && Array.isArray(v)) {
        ...
        v.forEach((t) => {
          if (t && typeof t === 'object') {
            ...
            if (hasExplicitDs) {
              ...
              if (typeof t.datasource === 'object' && t.datasource !== null) t.datasource.uid = datasourceUid
            } else {
        ...
        if (val.datasource && typeof val.datasource === 'string') {
          ...
          val.datasourceUid = val.datasource.uid || val.datasourceUid || val.datasource
        }
      ...
      if (key === 'templating' && v && typeof v === 'object') {
        ...
        if (!found) {
          ...
          v.list = [
            { name: 'ds_default', label: 'Datasource', type: 'datasource', current: { text: dsName, value: datasourceUid } },
            ...(v.list || []),
          ]
        }
        replaceInValue(v.list)
...
export function resolveToUid(candidate, availableDatasources = []) {
  ...
  const raw = (typeof candidate === 'object' && candidate !== null) ? (candidate.value || candidate.text || '') : String(candidate)
  if (!raw) return ''
  const byUid = availableDatasources.find(d => String(d.uid) === String(raw))
  if (byUid) return byUid.uid
  const byName = availableDatasources.find(d => String(d.name) === String(raw))
...
export function inferDashboardDatasource(dashboard, availableDatasources = []) {
  ...
  const resolvedPrimary = resolveToUid(primary, availableDatasources) || primary || ''
...
export function findPrimaryDatasourceUid(dashboard) {
  ...
  for (const p of panels) {
    ...
    if (p.datasource && typeof p.datasource === 'object' && p.datasource.uid) add(p.datasource.uid)
    const targets = p.targets || []
    for (const t of targets) {
      if (!t) continue
      if (t.datasourceUid) add(t.datasourceUid)
      if (t.datasource && typeof t.datasource === 'string') add(t.datasource)
      if (t.datasource && typeof t.datasource === 'object' && t.datasource.uid) add(t.datasource.uid)
    }
  }

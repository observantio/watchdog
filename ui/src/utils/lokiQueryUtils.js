...
function collectTokensFromValues(values, stopwords, maxSamples, state, tokens) {
  for (const value of values) {
    ...
    const parts = getLogText(value[1])
      .toLowerCase()
      .split(/[^a-z0-9_+-]+/)
      .filter(Boolean);
    const cleaned = parts
...
function mapTopTerms(freq, maxTerms) {
  ...
  return arr.slice(0, maxTerms).map((item) => {
    ...
    if (item.term.includes("error")) {
      ...
      item.term.includes("timeout") ||
      item.term.includes("timedout")
    ) {
      icon = "schedule";
...
export function computeTopTermsFromResult(result, maxTerms = 8) {
  ...
  if (tokens.length === 0) {
    const fallbackTokens = collectTokensFromStreamLabels(
      result.data.result,
      stopwords,
    );
...
export function getVolumeValues(volumeResponse) {
  return (volumeResponse?.data?.result?.[0]?.values || []).map((entry) =>
    Number(entry[1]),
  );
}
...
export function buildSelectorFromFilters(
  filters,
  fallbackLabel = "service_name",
) {
  if (!filters?.length) {

...
describe("overrideDashboardDatasource", () => {
        ...
        list: [
          {
            name: "ds_default",
            type: "datasource",
            current: { text: "old", value: "old-uid" },
          },
        ],
      },
      panels: [
        {
          id: 1,
          datasource: "old-uid",
          targets: [{ expr: "up", datasource: "old-uid" }],
        },
        {
          id: 2,
          datasource: { uid: "old-uid", type: "prometheus" },
          targets: [{ expr: "rate", datasourceUid: "old-uid" }],
        },
      ],
    };

    const datasources = [
      { uid: "new-uid", name: "New DS", type: "prometheus" },
    ];
  ...
  it("does not inject templating when injectTemplating is false but still updates panels/targets", () => {
    ...
    const out = overrideDashboardDatasource(
      dashboard,
      "prom-uid",
      [{ uid: "prom-uid", name: "Prom" }],
      false,
    );
  ...
  it("adds panel.datasource and target.datasource for targets with expr when missing", () => {
    ...
    const out = overrideDashboardDatasource(dashboard, "prom-uid", [
      { uid: "prom-uid", name: "Prom" },
    ]);
  ...
  it("inferDashboardDatasource prefers templating then falls back to panels", () => {
    ...
    const dashWithTempl = {
      templating: {
        list: [{ type: "datasource", current: { value: "Prom" } }],
      },
    };
    expect(inferDashboardDatasource(dashWithTempl, datasources)).toEqual({
      uid: "prom-uid",
      useTemplating: true,
    });

    const dashWithPanels = {
      panels: [{ datasourceUid: "x" }, { targets: [{ datasourceUid: "x" }] }],
    };
    expect(inferDashboardDatasource(dashWithPanels, datasources)).toEqual({
      uid: "x",
      useTemplating: false,
    });
  });
    ...
    const dashboard = {
      panels: [
        { id: 1, datasourceUid: "a", targets: [{ datasourceUid: "a" }] },
        {
          id: 2,
          datasource: "b",
          targets: [{ datasource: "b" }, { datasourceUid: "a" }],
        },
      ],
    };

...
import {
  ...
  getSpanAttribute,
  percentile,
  hasSpanError as spanHasErrorUtil,
} from "./helpers";
...
export function buildServiceGraphNodes(graphData, activeNodeId, hoverNodeId) {
  const nodes = [];
  const entries = Array.from(graphData.services.entries()).sort((a, b) =>
  ...
  entries.forEach(([name, stats]) => {
    ...
    const isConnected = activeNodeId
      ? graphData.edges.has(`${name}->${activeNodeId}`) ||
        graphData.edges.has(`${activeNodeId}->${name}`)
      : true;
...
export function buildServiceGraphInsights(graphData) {
  const serviceStats = Array.from(graphData.services.entries()).map(
    ([name, stats]) => {
      const p95 = percentile(stats.durations, 0.95);
      const errorRateNum = stats.spans ? (stats.errors / stats.spans) * 100 : 0;
      return {
  ...
  const painServices = serviceStats
    .filter(
      (service) =>
        service.p95 > PAIN_P95_THRESHOLD_US || service.errorRateNum > 5,
    )
    .sort((a, b) => b.p95 + b.errorRateNum - (a.p95 + a.errorRateNum))
    .slice(0, 3);

  const topCalls = [...edgeStats].sort((a, b) => b.count - a.count).slice(0, 3);
  const topErrors = [...edgeStats]
    .sort((a, b) => b.errorRateNum - a.errorRateNum)
    .slice(0, 3);

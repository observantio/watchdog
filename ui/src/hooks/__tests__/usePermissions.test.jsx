...
describe('usePermissions', () => {
  ...
  it('evaluates specific permission sets for regular users', () => {
    useAuth.mockReturnValue({ user: { is_superuser: false, permissions: ['read:alerts', 'write:alerts'] } })
    const { result } = renderHook(() => usePermissions())

    expect(result.current.hasPermission('read:alerts')).toBe(true)
    expect(result.current.hasPermission('delete:alerts')).toBe(false)
    expect(result.current.hasAnyPermission(['delete:alerts', 'read:alerts'])).toBe(true)
    expect(result.current.hasAllPermissions(['read:alerts', 'write:alerts'])).toBe(true)

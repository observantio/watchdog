...
export default function AuditCompliancePage() {
  ...
  const loadAudit = useCallback(
    async (baseFilters, { commit = true } = {}) => {
      ...
      try {
        ...
        toast.error(
          err?.body?.detail || err?.message || "Failed to load audit logs",
        );
        if (commit) {
          setItems([]);
          setHasMore(false);
  ...
  const applyPage = useCallback(
    async (nextFilters, { allowEmpty = true } = {}) => {
  ...
  const handleExportCsv = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        err?.body?.detail || err?.message || "Failed to export audit logs",
      );
    } finally {
      setExporting(false);
    }
  ...
  const pageEnd = items.length
    ? Math.min(filters.offset + items.length, filters.offset + filters.limit)
    : 0;
      ...
      items.map((row) => {
        ...
        return (
          ...
          >
            ...
            <td className="py-3 px-4 align-middle truncate">
              {userLabelById[row.user_id] ||
                row.username ||
                row.user_id ||
                "system"}
            </td>
            <td className="py-3 px-4 font-medium align-middle truncate">
              {highlight(row.action || "-", filters.q)}
            </td>
            <td className="py-3 px-4 align-middle truncate max-w-[240px]">
              {resource}
            </td>
            <td className="py-3 px-4 align-middle">
              {method && (
                <span className="inline-block px-2 py-0.5 mr-2 rounded text-xs bg-sre-surface-variant">
                  {method}
                </span>
              )}
              {status && (
            ...
            <td
              className="py-3 px-4 align-middle max-w-[420px] truncate"
              title={detailsTitle}
            >
              <div className="flex items-center gap-2">
                <div className="truncate text-xs text-sre-text-muted flex-1">
                  {highlight(detailsText, filters.q)}
                </div>
  ...
  if (!canView) {
    return (
      <div className="text-center py-12 text-sre-text-muted">
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto">
      <PageHeader
        ...
        subtitle="Who viewed what, who changed resources, and token lifecycle events."
      />
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {filters.user_id && (
            <Badge variant="info" className="px-2">
              User: {userLabelById[filters.user_id] || filters.user_id}{" "}
              <button onClick={() => clearFilter("user_id")} className="ml-2">
                ✕
              </button>
            </Badge>
          )}
          {filters.action && (
            <Badge className="px-2">
              ...
              <button onClick={() => clearFilter("action")} className="ml-2">
                ✕
              </button>
            </Badge>
          )}
          {filters.resource_type && (
            <Badge className="px-2">
              ...
              <button
                onClick={() => clearFilter("resource_type")}
                className="ml-2"
              >
                ✕
              </button>
          ...
          {filters.q && (
            <Badge className="px-2">
              Search: {filters.q}{" "}
              <button onClick={() => clearFilter("q")} className="ml-2">
                ✕
              </button>
            </Badge>
          )}
              ...
              <tbody className="divide-y">
                ...
                {items.length === 0 && (
                  <tr>
                    <td
                      colSpan={7}
                      className="py-12 px-4 text-sre-text-muted text-center"
                    >
                      No audit records found for the current filters.
                      <div className="mt-3 text-xs">
                        Try widening the date range, removing filters, or
      ...
      {selected && (
        ...
        >
          <div
            className="bg-sre-surface border border-sre-border rounded-2xl shadow-lg max-w-6xl w-full p-0 m-4 overflow-hidden"
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-5 border-b border-sre-border bg-sre-surface/70">
              <div className="flex items-start gap-4 min-w-0">
                <div className="w-12 h-12 rounded-lg bg-sre-primary/10 text-sre-primary flex items-center justify-center font-semibold text-lg flex-shrink-0">
                  {(
                    userLabelById[selected.user_id] ||
                    selected.username ||
                    "system"
                  )
                    .slice(0, 1)
                ...
                <div className="min-w-0">
                  <h3 className="text-lg font-semibold text-sre-text truncate">
                    {selected.action}
                  </h3>
                  <div className="mt-1 text-xs text-sre-text-muted truncate">
                    {formatLocal(selected.created_at)} —{" "}
                    {userLabelById[selected.user_id] ||
                      selected.username ||
                      "system"}
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    {selected.details?.method && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-sre-surface-variant text-sre-text-muted">
                        {selected.details.method}
                      </span>
                    )}
                    {selected.details?.status_code && (
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${Number(selected.details.status_code) >= 500 ? "bg-red-100 text-red-800" : Number(selected.details.status_code) >= 400 ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}`}
                      >
                        {selected.details.status_code}
                      </span>
                    )}
                    {selected.details?.query !== undefined && (
                      <span className="text-xs text-sre-text-subtle ml-2">
                        query: "{String(selected.details.query)}"
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="flex gap-2 items-center">
                <div className="flex items-center gap-2 whitespace-nowrap">
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      copyText(prettyJson(selected.details || {}));
                    }}
              ...
              <div className="md:col-span-2 space-y-4">
                <div className="bg-sre-bg/30 border border-sre-border rounded-lg p-4">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        className="text-xs text-sre-text-muted hover:text-sre-text"
                        onClick={(e) => {
                          e.stopPropagation();
                          copyText(
                            (selected.resource_type || "") +
                      ...
                      {(
                        (selected.resource_type || "").toLowerCase() || ""
                      ).startsWith("http") && (
                        <a
                ...
                <div className="bg-sre-bg/30 border border-sre-border rounded-lg p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="mt-2 text-xs text-sre-text-muted">
                        Full JSON payload (copy available)
                      </div>
                    </div>
                    <div>
                      <button
                  ...
                  <pre className="mt-3 bg-sre-surface rounded p-3 overflow-auto text-xs max-h-80 border border-sre-border font-mono">
                    {prettyJson(selected.details)}
                  </pre>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-sre-bg/30 border border-sre-border rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-md bg-sre-primary/10 text-sre-primary flex items-center justify-center font-semibold text-sm">
                      {(
                        userLabelById[selected.user_id] ||
                        selected.username ||
                        "system"
                ...
                <div className="bg-sre-bg/30 border border-sre-border rounded-lg p-4">
                  ...
                  <div className="text-xs text-sre-text-muted font-mono break-words max-h-40 overflow-auto">
                    {selected.user_agent || "-"}
                  </div>
                </div>
              </div>

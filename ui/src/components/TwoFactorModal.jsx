...
export default function TwoFactorModal({
  ...
  const startEnroll = async () => {
    ...
    try {
      ...
      setError(
        err?.body?.detail || err?.message || "Failed to start 2FA enrollment",
      );
      setEnrolling(false);
    }
  };
  ...
  const disable = async () => {
    setError("");
    const current_password = window.prompt(
        ...
        {!user?.mfa_enabled && (
          <div>
            <p className="text-sm text-sre-text-muted mb-3">
              Protect your account by enabling an authenticator app (TOTP). You
              will be shown a secret and a QR code to scan.
            </p>
            {!otpauthUrl && (
              <div className="flex gap-2">
                <Button onClick={startEnroll} loading={enrolling}>
                  Enable 2FA
                </Button>
              </div>
            )}

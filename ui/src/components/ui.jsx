      ...
      {(title || action) && (
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            {subtitle && (
              <p className="text-sm text-sre-text-muted mt-1 text-left">
                {subtitle}
              </p>
            )}
          </div>
...
export function CardTitle({ children, className, ...props }) {
  return (
    <h3
      className={clsx("text-lg font-semibold text-sre-text", className)}
      {...props}
    >
      {children}
    </h3>
  ...
  return (
    <div className="w-full">
      ...
      <input
        ...
        {...props}
      />
      {error && <p className="mt-1 text-sm text-sre-error">{error}</p>}
      {helperText && !error && (
...
export function Sparkline({
  ...
  const areaPoints =
    fill === "none" ? points : `0,${height} ${points} ${width},${height}`;
  ...
  return (
    ...
    >
      {fill !== "none" && (
        <polygon fill={fill} points={areaPoints} opacity="0.3" />
      )}
      <polyline
        fill="none"
        stroke={stroke}
      ...
      >
        ...
        {(title || showCloseButton) && (
          <div className="flex items-center justify-between px-6 py-4 border-sre-border">
            <h2
              id="modal-title"
              className="text-xl font-bold text-sre-text mb-0"
            >
              {title}
            </h2>
            {showCloseButton && (

    ...
    <>
      <div>
        ...
        <Input
          type="email"
          value={config.to || ''}
          onChange={(e) => setEmailConfig(formData, setFormData, { to: e.target.value })}
      ...
      <div>
        <label className="block text-sm font-medium text-sre-text mb-2">
          Delivery Method <HelpTooltip text="Choose SMTP or API-based delivery provider." />
        </label>
        <Select
          value={provider}
          onChange={(e) => setEmailConfig(formData, setFormData, {
      ...
      <div>
        <label className="block text-sm font-medium text-sre-text mb-2">
          From address <HelpTooltip text="Sender address used in outgoing emails." />
        </label>
        <Input
          type="email"
      ...
      {provider === 'smtp' && (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-sre-text mb-2">
              SMTP Auth Method <HelpTooltip text="Use password auth, API key auth, or no auth for trusted relays." />
            </label>
            <Select
              value={authType}
              onChange={(e) => setEmailConfig(formData, setFormData, {
          ...
          {authType !== 'none' && (
            <div>
              <label className="block text-sm font-medium text-sre-text mb-2">
                SMTP Username <HelpTooltip text="Username for SMTP auth. For many API-key SMTP providers, use 'apikey'." />
              </label>
              <Input
          ...
          {authType === 'password' && (
            <div>
              <label className="block text-sm font-medium text-sre-text mb-2">
                SMTP Password <HelpTooltip text="Password for SMTP authentication." />
              </label>
              <Input
                type="password"
          ...
          {authType === 'api_key' && (
            <div>
              <label className="block text-sm font-medium text-sre-text mb-2">
                SMTP API Key <HelpTooltip text="API key used as SMTP auth secret (for example, SendGrid SMTP relay)." />
              </label>
              <Input
                type="password"
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={!!(config.smtpStartTLS || config.smtp_starttls || config.starttls)}
                onChange={(e) => setEmailConfig(formData, setFormData, {
      ...
      {provider === 'sendgrid' && (
        <div>
          <label className="block text-sm font-medium text-sre-text mb-2">
            SendGrid API Key <HelpTooltip text="API key used for SendGrid Web API v3 email delivery." />
          </label>
          <Input
            type="password"
            value={config.sendgridApiKey || config.sendgrid_api_key || config.apiKey || config.api_key || ''}
            onChange={(e) => setEmailConfig(formData, setFormData, {
      ...
      {provider === 'resend' && (
        <div>
          <label className="block text-sm font-medium text-sre-text mb-2">
            Resend API Key <HelpTooltip text="API key used for Resend email API delivery." />
          </label>
          <Input
            type="password"
            value={config.resendApiKey || config.resend_api_key || config.apiKey || config.api_key || ''}
            onChange={(e) => setEmailConfig(formData, setFormData, {

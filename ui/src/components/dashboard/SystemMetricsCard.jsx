        ...
        <div
          className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
            systemMetrics.stress?.status === "stressed"
              ...
              : systemMetrics.stress?.status === "moderate"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 rounded-lg bg-sre-bg-alt border border-sre-border">
          <div className="flex items-center gap-2 mb-2">
            <svg
              className="w-4 h-4 text-sre-primary"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="text-xs text-sre-text-muted mt-1">
            {systemMetrics.cpu?.threads} threads
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg bg-sre-bg-alt border border-sre-border">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-sre-text-muted">
              Memory
            </span>
          </div>
          <div className="text-lg font-bold text-sre-text">
            {systemMetrics.memory?.utilization?.toFixed(1)}%
          </div>
          <div className="text-xs text-sre-text-muted mt-1">
            RSS: {systemMetrics.memory?.rss_mb} MB
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg bg-sre-bg-alt border border-sre-border">
          ...
          <div className="text-lg font-bold text-sre-text">
            {(systemMetrics.io?.read_mb + systemMetrics.io?.write_mb)?.toFixed(
              1,
            )}{" "}
            MB
          </div>
          <div className="text-xs text-sre-text-muted mt-1">
            ↑{systemMetrics.io?.write_mb} ↓{systemMetrics.io?.read_mb}
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg bg-sre-bg-alt border border-sre-border">
          ...
          <div className="text-lg font-bold text-sre-text">
            {systemMetrics.network?.total_connections || 0}
          </div>
          <div className="text-xs text-sre-text-muted mt-1">
            {systemMetrics.network?.established || 0} active
          </div>
        </div>
      </div>
        ...
        <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
          ...
          <ul className="space-y-1">
            {systemMetrics.stress.issues.map((issue) => (
              <li key={issue} className="text-xs text-sre-text-muted">
                • {issue}
              </li>
            ))}
          </ul>

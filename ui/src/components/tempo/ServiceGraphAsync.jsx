...
const Stat = ({ label, value, highlight }) => (
  <div className="flex flex-col gap-0.5">
    ...
    <span
      className={`text-xs font-semibold tabular-nums ${highlight ?? "text-sre-text"}`}
    >
      {value}
    </span>
  </div>
...
const ServiceNode = ({ data }) => {
  ...
  const p95Hi =
    stats.p95?.includes("ms") && parseFloat(stats.p95) > 1000
      ? "text-red-400"
      : undefined;
  const errHi =
    errorRate > 5
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2.5">
        ...
        />
        <span className="font-semibold text-sre-text text-sm truncate flex-1 leading-tight">
          {name}
        </span>
        <div className="flex items-center gap-1 flex-shrink-0">
          {stats.inbound === 0 && (
            <span className="px-1.5 py-0.5 text-[9px] font-bold rounded-full bg-sre-primary/20 text-sre-primary border border-sre-primary/30 uppercase tracking-wide">
              Entry
            </span>
          )}
          {stats.outbound === 0 && (
            <span className="px-1.5 py-0.5 text-[9px] font-bold rounded-full bg-sre-surface text-sre-text-muted border border-sre-border/50 uppercase tracking-wide">
              Leaf
            </span>
          )}
        </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[9px] text-sre-text-muted uppercase tracking-wide">
          Health
        </span>
      </div>
      ...
      {isPain && (
        <div
          className={`mt-2 flex items-center gap-1.5 text-[10px] px-2 py-1 rounded-lg ${c.soft} text-red-300 border border-red-500/20`}
        >
...
const StatCard = ({ label, children, empty }) => (
  <div className="p-3 rounded-xl border border-sre-border/40 bg-sre-bg/40 flex flex-col gap-1.5 min-w-0">
    <div className="text-[10px] font-semibold uppercase tracking-widest text-sre-text-muted">
      {label}
    </div>
    {empty ? (
      <div className="text-xs text-sre-text-muted/60 italic">{empty}</div>
    ) : (
      children
    )}
  </div>
...
export default function ServiceGraphAsync({ traces }) {
  ...
  const graphData = useMemo(
    () =>
      hasSpanData
        ? buildServiceGraphData(traces)
        : { services: new Map(), edges: new Map() },
    [traces, hasSpanData],
  );

  const structuralNodes = useMemo(
    () => buildServiceGraphNodes(graphData, null, null),
    [graphData],
  );
  const structuralEdges = useMemo(
    () => toEdges(buildServiceGraphEdges(graphData, null, null)),
    [graphData],
  );
  const insights = useMemo(
    () => buildServiceGraphInsights(graphData),
    [graphData],
  );
    ...
    setEdges((prev) =>
      prev.map((e) => {
        ...
        const isHighlighted = activeNodeId
          ...
          : false;
        const dimmed =
          (activeEdgeId || activeNodeId) && !isActive && !isHighlighted;
        const color = EDGE_COLOR(e.data?.errorRateNum ?? 0);
        return {
          ...e,
          selected: isActive,
  ...
  const handleEdgeClick = useCallback((_, e) => {
  ...
  if (layouted.nodes.length === 0) {
    return !hasSpanData ? (
      <div className="border-2 border-dashed border-sre-border/40 rounded-2xl p-10 text-center">
        <span className="material-icons text-4xl text-sre-text-muted/40 block mb-3">
          hub
        </span>
        <p className="text-sm font-medium text-sre-text mb-1">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-sre-text leading-none">
                Service Dependency Map
              </h3>
              <p className="text-[11px] text-sre-text-muted mt-0.5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_240px] gap-3">
          <div className="grid grid-cols-3 gap-2">
            <StatCard
              label="Top Pain Services"
              empty={
                !insights.painServices.length ? "No pain points" : undefined
              }
            >
              {insights.painServices.map((s) => (
                ...
                >
                  ...
                  <span className="text-red-400 flex-shrink-0 tabular-nums">
            ...
            <StatCard
              label="Busiest Flows"
              empty={!insights.topCalls.length ? "No flows" : undefined}
            >
              {insights.topCalls.map((e) => (
                ...
                >
                  ...
                  <span className="text-sre-text-muted flex-shrink-0 tabular-nums">
                    {e.count}
                  </span>
                </button>
              ))}
            ...
            <StatCard
              ...
              empty={!insights.topErrors.length ? "No errors" : undefined}
            >
            ...
            {activeNode && (
              <div className="flex flex-col gap-1.5">
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor: STATUS_COLOR(
                        activeNode.errorRateNum,
                        activeNode.pain,
                      ).hex,
                    }}
                  />
                  <span className="text-sm font-semibold text-sre-text truncate">
                    {activeNode.name}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
                  ...
                  <Stat
                    label="Error"
                    value={`${activeNode.errorRateNum.toFixed(1)}%`}
                  />
                  <Stat
                    label="In/Out"
                    value={`${activeNode.inbound}/${activeNode.outbound}`}
                  />
                </div>
              </div>
            )}
            {activeEdge && (
              <div className="flex flex-col gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
                  ...
                  <Stat
                    label="Error"
                    value={`${activeEdge.errorRateNum.toFixed(1)}%`}
                  />
                </div>
              </div>
            )}
            {!activeNode && !activeEdge && (
              <p className="text-xs text-sre-text-muted/60 italic mt-1">

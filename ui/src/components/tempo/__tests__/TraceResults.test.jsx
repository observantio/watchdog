...
describe('TraceResults / TraceCard counts', () => {
  it('shows correct span and service counts for summary traces', () => {
    const sample = {
      traceID: 'abc',
      spans: [
        { spanID: 'root', serviceName: 'svc1', operationName: 'op', startTime: 0, duration: 1 }
      ]
    }
    const { getByText } = render(<TraceResults traces={[sample]} loading={false} handleTraceClick={() => {}} />)
  ...
  it('ignores unknown service names when counting services', () => {
    ...
    const { getByText, queryByText } = render(<TraceResults traces={[sample]} loading={false} handleTraceClick={() => {}} />)
    expect(getByText(/2 spans/i)).toBeInTheDocument()

...
export default function PermissionEditor({ user, groups, onClose, onSave }) {
  ...
  const getPermissionDescription = (permissionName) => {
    const descriptions = {
      ...
      "View OTEL agents and system metrics":
        "View OTEL agents and system metrics",
      ...
      "View alert rules and active alerts":
        "View alert rules and active alerts",
  ...
  useEffect(() => {
    ...
    const directPermsSource = hasDirectPermissions
      ? user.direct_permissions || []
      : [];
    const directPerms = directPermsSource.map((p) =>
  ...
  const selectAllInCategory = (category) => {
    const categoryPerms = (permissionsByCategory[category] || []).map(
      (p) => p.name || p.id,
    );
    const newPerms = new Set(selectedPermissions);
        ...
        <div className="space-y-6 overflow-y-auto pr-1">
          ...
          <div className="flex items-start gap-2">
            <div className="flex-1">
              <label
                htmlFor="role"
                className="block text-sm font-semibold text-sre-text mb-2"
              >
                Role
              </label>
              <select
                ...
                <div key={group.id} className="">
                  <div className="flex items-start gap-3 p-3 bg-sre-bg-alt border border-sre-border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      {group.description && (
                        <div
                          className="text-xs text-sre-text-muted mt-1 truncate"
                  ...
                  {expandedGroups.has(group.id) && (
                    <Card className="!p-3 ml-6 mt-2 rounded-lg border border-sre-border">
                      <div className="text-sm font-semibold text-sre-text mb-2">
                        Group Permissions
                      </div>
                      <div className="space-y-2">
                        {(group.permissions || []).length === 0 && (
                          <div className="text-xs text-sre-text-muted">
                            No explicit permissions on this group
                          </div>
                        )}
                        {(group.permissions || []).map((perm) => {
                          const name =
                            typeof perm === "string"
                              ? perm
                              : perm.display_name || perm.name || perm.id;
                          const desc =
                            typeof perm === "string"
                              ? ""
                              : perm.description || "";
                ...
                Object.entries(permissionsByCategory).map(
                  ([category, perms]) => {
                    const allSelected = perms.every((p) =>
                      selectedPermissions.has(p.name || p.id),
                    );
                      ...
                      <Card key={category} className="!p-4">
                        ...
                        <div className="space-y-2">
                          {perms.map((perm) => {
                            ...
                            const isFromRoleOrGroup =
                              computedPermissions.has(permName) &&
                              !isDirectlySet;

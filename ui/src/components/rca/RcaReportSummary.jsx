...
export default function RcaReportSummary({ report, compact = false }) {
  ...
  const gatingProfileShort = gatingProfile.length > 18 ? `${gatingProfile.slice(0, 15)}...` : gatingProfile
  const suppressionTotal = quality?.suppression_counts
    ? Object.values(quality.suppression_counts).reduce((acc, n) => acc + Number(n || 0), 0)
    : 0
  const densityValues = quality?.anomaly_density ? Object.values(quality.anomaly_density).map((v) => Number(v || 0)) : []
  ...
  const content = (
    <>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {maxDensity !== null && (
          <div className="flex-1 min-w-[160px]"><MetricCard label="Max Density/hr" value={maxDensity.toFixed(2)} status={maxDensity > 1 ? 'warning' : 'success'} /></div>
        )}
      </div>
    </>
  )

...
export default function RcaForecastSloPanel({
  report,
  forecast,
  slo,
  compact = false,
        ...
        <TableSection
          ...
          renderRow={(row) => (
            <>
              ...
              <td
                className={`px-3 py-2 uppercase ${statusClass(row.severity)}`}
              >
                {row.severity || "-"}
              </td>
        ...
        <TableSection
          ...
          renderRow={(row) => (
            <>
              ...
              <td
                className={`px-3 py-2 ${row.forecast ? statusClass(row.forecast?.severity) : "text-emerald-300"}`}
              >
                {row.forecast
              ...
              <td
                className={`px-3 py-2 ${row.degradation ? statusClass(row.degradation?.severity) : "text-emerald-300"}`}
              >
                {row.degradation
        ...
        <div className="border border-sre-border rounded-xl bg-sre-surface/20 p-3">
          ...
          {budgetStatus ? (
            <div className="space-y-2">
              ...
              <p className="text-xs text-sre-text-muted">
                Target availability:{" "}
                <span className="text-sre-text font-mono">
                  {fmt(budgetStatus.target_availability, 4)}
                </span>
              </p>
              <p className="text-xs text-sre-text-muted">
                Current availability:{" "}
                <span className="text-sre-text font-mono">
                  {fmt(budgetStatus.current_availability, 4)}
                </span>
              </p>
              <p className="text-xs text-sre-text-muted">
                Budget used:{" "}
                <span className="text-sre-text font-mono">
                  {fmt(budgetStatus.budget_used_pct)}%
                </span>
              </p>
              <p className="text-xs text-sre-text-muted">
                Remaining minutes:{" "}
                <span className="text-sre-text font-mono">
                  {fmt(budgetStatus.remaining_minutes, 1)}
                </span>
              </p>
            </div>
          ) : (
            <p className="text-xs text-sre-text-muted">
              No budget status returned.
            </p>
          )}
        </div>

        <div className="border border-sre-border rounded-xl bg-sre-surface/20 p-3">
          <h4 className="text-sm font-semibold text-sre-text mb-2">
            Degradation
          </h4>
          {degradationSignals.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-1.5">
              {degradationSignals.slice(0, 20).map((signal, index) => (
                <span
                  key={`${signal?.metric_name || signal?.metric || "signal"}-${index}`}
                  className="text-xs px-2 py-1 rounded-md border border-sre-border bg-sre-surface"
                >

...
export default function RcaCausalPanel({ granger, bayesian, mlWeights, deployments }) {
  ...
  const posteriors = (bayesian?.posteriors || [])
    .slice()
    .sort((left, right) => Number(right.posterior || 0) - Number(left.posterior || 0))
    ...
    <Card className="border border-sre-border p-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <TableCard
          ...
          rows={grangerPairs}
          rowKey={(row, index) => `${row.cause_metric || row.cause}-${row.effect_metric || row.effect}-${index}`}
          emptyText="No causal pairs returned in this range."
          renderRow={(row) => (
            <>
              <td className="px-3 py-2 text-sre-text">{row.cause_metric || row.cause || '-'}</td>
              <td className="px-3 py-2 text-sre-text">{row.effect_metric || row.effect || '-'}</td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">{formatNumber(row.strength, 4)}</td>
            </>
          )}
        />
        ...
        <TableCard
          ...
          renderRow={(row) => (
            <>
              ...
              <td className="px-3 py-2 text-sre-text-muted font-mono">{formatNumber(row.posterior, 4)}</td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">{formatNumber(row.prior, 4)}</td>
            </>
          )}
        />
          ...
          renderRow={(row) => {
            ...
            return (
              <>
                ...
                <td className="px-3 py-2 text-sre-text-muted font-mono">{formatNumber(row.value, 4)}</td>
                <td className="px-3 py-2">
                  <div className="h-2 rounded-full bg-sre-border/40 overflow-hidden">
                    <div className="h-full rounded-full bg-sre-primary/80" style={{ width: `${width}%` }} />
                  </div>
                </td>
              </>
            )
        ...
        <TableCard
          ...
          rowKey={(row, index) => `${row.service || 'service'}-${row.timestamp || 'ts'}-${index}`}
          emptyText="No deployment events found for this scope."
          renderRow={(row) => (
            <>
              <td className="px-3 py-2 text-sre-text">{row.service || '-'}</td>
              <td className="px-3 py-2 text-sre-text-muted">{row.version || '-'}</td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {row.timestamp ? new Date(Number(row.timestamp) * 1000).toLocaleString() : '-'}
              </td>
            </>
          )}
        />

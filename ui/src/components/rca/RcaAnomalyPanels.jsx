    ...
    <>
      ...
      >
          ...
          renderRow={(row) => (
            <>
              ...
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {asTime(row.timestamp)}
              </td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {Number(row.value || 0).toLocaleString(undefined, {
        ...
        <DataSection
          ...
          rows={logBursts}
          rowKey={(row, index) =>
            `${row.window_start}-${row.window_end}-${index}`
          }
          empty="No log bursts."
          renderRow={(row) => (
            <>
              <td className="px-3 py-2 text-sre-text-muted">
                {asTime(row.window_start)} - {asTime(row.window_end)}
              </td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {Number(row.rate_per_second || 0).toFixed(2)}
              </td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {Number(row.baseline_rate || 0).toFixed(2)}
              </td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">
        ...
        <DataSection
          ...
          renderRow={(row) => (
            <>
              <td
                className="px-3 py-2 text-sre-text truncate max-w-[270px]"
                title={row.pattern || ""}
              >
                {row.pattern || "-"}
              </td>
              ...
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {Number(row.rate_per_minute || 0).toFixed(2)}
              </td>
              <td
                className={`px-3 py-2 uppercase ${severityClass(row.severity)}`}
              >
        ...
        <DataSection
          ...
          renderRow={(row) => (
            <>
              ...
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {Number(row.p95_ms || 0).toFixed(1)}
              </td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {Number(row.p99_ms || 0).toFixed(1)}
              </td>
              <td className="px-3 py-2 text-sre-text-muted font-mono">
                {Number(row.apdex || 0).toFixed(3)}
              </td>
            </>
          )}
        />
      ...
      <div className="mt-3 border border-sre-border rounded-xl bg-sre-surface/20 overflow-hidden">
        <div className="px-3 py-2 border-b border-sre-border bg-sre-surface/40 flex items-center justify-between">
          ...
          <span className="text-[11px] text-sre-text-muted">
            {errorPropagation.length}
          </span>
        </div>
        {errorPropagation.length === 0 ? (
          <p className="p-4 text-xs text-sre-text-muted">
          ...
          <div className="p-3 space-y-2">
            {errorPropagation.slice(0, 40).map((entry, index) => (
              <div
                key={`${entry.source_service}-${index}`}
                className="rounded-lg border border-sre-border/50 bg-sre-surface/25 p-2"
              >
                <p className="text-xs font-semibold text-sre-text">
                  {entry.source_service || "unknown source"}
                </p>
                <p className="text-xs text-sre-text-muted mt-1">
                  Impacted:{" "}
                  {(entry.affected_services || []).join(", ") || "none"}
                </p>
                <p className={`text-xs mt-1 ${severityClass(entry.severity)}`}>
                  error_rate={Number(entry.error_rate || 0).toFixed(4)}{" "}

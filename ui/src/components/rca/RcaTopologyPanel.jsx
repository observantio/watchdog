...
function buildTopologyGraph(topology) {
  ...
  const downstream = Array.isArray(topology?.affected_downstream)
    ? topology.affected_downstream
    : [];
  const upstream = Array.isArray(topology?.upstream_roots)
    ? topology.upstream_roots
    : [];
  const allServices = Array.isArray(topology?.all_services)
    ? topology.all_services
    : [];

  const serviceSet = new Set(
    [root, ...downstream, ...upstream, ...allServices].filter(Boolean),
  );
  ...
  const related = all.filter(
    (service) =>
      service !== root &&
      !downstream.includes(service) &&
      !upstream.includes(service),
  );
...
export default function RcaTopologyPanel({ topology, compact = false }) {
    ...
    const emptyContent = (
      <>
        ...
        <p className="text-sm text-sre-text-muted">
          Topology data not available for this report.
        </p>
      </>
    );
  ...
  const content = (
    <>
      <h3 className="text-lg text-sre-text font-semibold mb-1">
        Topology and Blast Radius
      </h3>
      <p className="text-sm text-sre-text-muted mb-3">
        Root service:{" "}
        <span className="text-sre-text font-semibold">
          {topology.root_service || "n/a"}
        </span>
      </p>

      <div
        className={`grid ${compact ? "grid-cols-1" : "grid-cols-2 md:grid-cols-4"} gap-2 mb-3`}
      >
        <div className="rounded-lg border border-sre-border bg-sre-surface/25 p-2">
          <p className="text-[11px] uppercase tracking-wide text-sre-text-muted">
            Downstream
          </p>
          <p className="text-sm text-sre-text font-semibold">
            {(topology.affected_downstream || []).length}
          </p>
        </div>
        <div className="rounded-lg border border-sre-border bg-sre-surface/25 p-2">
          <p className="text-[11px] uppercase tracking-wide text-sre-text-muted">
          ...
          <p className="text-sm text-sre-text font-semibold">
            {(topology.upstream_roots || []).length}
          </p>
        </div>
        <div className="rounded-lg border border-sre-border bg-sre-surface/25 p-2">
          <p className="text-[11px] uppercase tracking-wide text-sre-text-muted">
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-xs text-sre-text-muted">
        <span className="px-2 py-1 rounded-md border border-sre-border bg-sre-primary/10 text-sre-primary">
          Root
        </span>
        <span className="px-2 py-1 rounded-md border border-sre-border bg-red-500/10 text-red-300">
          Downstream
        </span>
        <span className="px-2 py-1 rounded-md border border-sre-border bg-emerald-500/10 text-emerald-300">
          Upstream Root
        </span>
        <span className="px-2 py-1 rounded-md border border-sre-border bg-slate-400/10 text-slate-300">
          Related Service
        </span>
      </div>
    </>
